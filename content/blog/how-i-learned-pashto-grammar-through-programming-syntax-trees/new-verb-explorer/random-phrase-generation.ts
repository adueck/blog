...
export function generateRandomNP(): T.NPSelection {
  if (!Array.isArray(entryFeeder.nouns)) {
    throw new Error(
      "need to use array type entry feeder for random phrase generation",
    );
  }
  const types: ("noun" | "participle" | "pronoun")[] = [
    "noun",
    "participle",
    "pronoun",
    "noun",
...
function generateRandomNoun(): T.NounSelection {
  ...
  const adjectives = randFromArray([
    [],
    [],
    [],
    [1],
  ...
  return {
    ...np,
    gender: np.genderCanChange ? randFromArray(["masc", "fem"]) : np.gender,
