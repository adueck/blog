import { typePredicates, Types as T } from "@lingdocs/pashto-inflector";
const {
  isPattern1Entry,
  isPattern2Entry,
  isPattern3Entry,
...
export function categorize<I, X extends Record<string, I[]>>(
  ...
  const o: X = Object.keys(preds).reduce(
    (all, curr) => ({
      ...all,
      [curr]: [],
    }),
  ...
  const lk = Object.entries(preds).find(
    ([key, entry]) => entry === "leftovers",
  );
...
export function intoPatterns<T extends T.NounEntry | T.AdjectiveEntry>(
  words: T[],
): {
